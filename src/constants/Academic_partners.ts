import {
  Eco,
  IAFIE,
  IEI,
  sponsorAlgStock,
  sponsorIstanbul,
  Logo2,
  HIC,
  partnerI3jaz
} from "@/assets";

export const academic_partners = [
  {
    name: "Faculty of Economics, Commerce, and Management Sciences",
    logo: Eco,
    website: "https://eco.univ-setif.dz/",
    description:
      "Faculty of Economics, Commerce, and Management Sciences at Setif 1 University, providing quality education and research in economic sciences.",
  },
  {
    name: "Setif 1, University Ferhat Abbas",
    logo: Logo2,
    website: "https://www.univ-setif.dz/",
    description:
      "Setif 1, University Ferhat Abbas, a leading university in Algeria offering diverse academic programs and research opportunities.",
  },
  {
    name: "Istanbul Zaim University",
    logo: sponsorIstanbul,
    website: "https://www.izu.edu.tr/en",
    description:
      "Istanbul Zaim University, a prestigious institution in Turkey offering a wide range of academic programs and research initiatives.",
  },
  {
    name: "International Association for Islamic Economics",
    logo: IAFIE,
    website: "",
    description:
      "International Association for Islamic Economics, promoting the study and application of Islamic economics globally.",
  },
  {
    name: "High Islamic Council",
    logo: HIC,
    website: "http://www.elmadjlis-hci.dz/",
    description: "",
  },

  {
    name: "Islamic Economics Institute- JEDDAH",
    logo: IEI,
    website: "https://iei.kau.edu.sa/Default-121-EN",
    description:
      "Islamic Economics Institute at King Abdulaziz University, Jeddah, dedicated to research and education in Islamic economics.",
  },
  {
    name: "Authority for Scientific Miracles in the Qur’an, Algeria Office",
    logo: partnerI3jaz,
    website: "https://iei.kau.edu.sa/Default-121-EN",
    description:
      "Authority for Scientific Miracles in the Qur’an, Algeria Office",
  },

  {
    name: "Algiers Stock Exchange",
    logo: sponsorAlgStock,
    website: "https://www.sgbv.dz/?lang=eng",
    description:
      "Algiers Stock Exchange, the primary stock exchange in Algeria, facilitating trading and investment in the Algerian market.",
  },
];
